...
var appData = yaml.safeLoad('app_data', fs.readFileSync('/src/config/app_data.yml'));
...
var verifySignature = function(req, res, next, appSecret) {
  ...
  if (digest(resource + appSecret) === signature) {
    ...
    console.log(resource, appSecret, signature, digest(resource + appSecret));
